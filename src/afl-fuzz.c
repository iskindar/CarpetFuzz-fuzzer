...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "+Ab:B:c:CdDe:E:hi:I:f:F:g:G:l:K:L:m:M:nNOo:p:RQs:S:t:T:UV:WXx:YZ")) >
      0) {
    ...
    switch (opt) {
      ...
      // CarpetFuzz modified
      case 'K':   /* argvs list path (enable Fuzz argv) */
 
        if (afl->argvs_list_path) { FATAL("Multiple -K options not supported"); }
        afl->do_fuzz_with_argvs = true;
  ...
  // CarpetFuzz modified
  // afl->argv = use_argv;
  if(afl->fsrv.argvs_file == NULL) {
    afl->fsrv.argvs_file = alloc_printf("%s/.argvs", afl->tmp_dir);
  }
  afl->argv = (char **)calloc(3, sizeof(char *));
  afl->argv[0] = use_argv[0];
  afl->argv[1] = afl->fsrv.argvs_file;
  afl->argv[2] = NULL;

  OKF("argvs_file=%s", afl->fsrv.argvs_file);

  // initialize the global argvs idx state to 0
  afl->argvs_idx = 0;
  // write argvs[0] to argvs_file
  write_argvs_file(afl, afl->argvs_list[afl->argvs_idx]->argv_buf);

  u32 idx1;
  // init the queued seeds with argvs[0]
  for(idx1 = 0; idx1 < afl->queued_items; ++idx1) {
    struct queue_entry *q = afl->queue_buf[idx1];
    q->argvs_idx = afl->argvs_idx;
  }
  OKF("initialize argvs[0] for queued seeds");
  // CarpetFuzz end
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      // CarpetFuzz modified
      /* Fuzz with argvs traversing, with unlikely optimization */
      if (unlikely(afl->fuzz_argv_done == false)) {
        fuzz_one_with_argvs(afl);
        afl->fuzz_argv_done = true;
      }

      /* Fuzz argv related logic */
      // keep global argvs_idx state up with argvs_idx in queue_cur
      afl->argvs_idx = afl->queue_cur->argvs_idx;
      
      // write current argvs to .argvs file
      write_argvs_file(afl, afl->argvs_list[afl->argvs_idx]->argv_buf);
      // CarpetFuzz end
