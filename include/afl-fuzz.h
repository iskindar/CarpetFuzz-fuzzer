...
typedef struct afl_state {
  ...
  u8 skip_deterministic,                /* Skip deterministic stages?       */
      ...
      dict_only;                       /* use the dictionary only in deterministic  */
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  // CarpetFuzz modified
  bool do_fuzz_with_argvs;        /* do fuzz with argvs traversing */
  bool fuzz_argv_done;            /* fuzz_one_with_argv done? */
  u8 *argvs_list_path;
  struct argv_list **argvs_list;  /* each group includes OPTION, excludes BIN and @@ */
  u32 argvs_num;
  u32 argvs_idx;                  /* argvs idx global state */
  // CarpetFuzz end
} afl_state_t;
...
void print_argv(char **, char *);
void init_argv_list(afl_state_t*, u8 *);
void write_argvs_file(afl_state_t *, char **);
void fuzz_one_with_argvs(afl_state_t *);
// CarpetFuzz end
